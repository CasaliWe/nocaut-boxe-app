...
<div class="modal fade" id="modalAviso-backup" tabindex="-1" aria-labelledby="modalAviso-backup" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content px-3">
                <div class="py-4 modal-header d-flex flex-column">
                    <h6 class="modal-title fs-5 mb-4 text-center" id="exampleModalLabel">Backup feito com sucesso! <i class="ms-2 fas fa-check text-success"></i> </h6>
                    <button onclick="reescreverRota('backup')" type="button" class="w-25 btn btn-success" data-bs-dismiss="modal" aria-label="Close"> Fechar </button>
                </div>
        </div>
    </div>
</div>
...
<?php if(isset($_GET['backup']) && $_GET['backup'] == "true"){ ?>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
        var modalVaga = new bootstrap.Modal(document.getElementById('modalAviso-backup'));
        modalVaga.show();
        });
    </script>
<?php } ?>
